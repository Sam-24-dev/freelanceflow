(function proposalModelFactory(globalScope) {
  ...
  function hasValidLegacyRawValues(proposal) { return isPlainObject(proposal) && CURRENCIES.includes(text(proposal.moneda).toUpperCase()) && validRawMoney(proposal.descuento) && Array.isArray(proposal.items) && proposal.items.every((item) => isPlainObject(item) && validRawNumber(item.cantidad) && validRawMoney(item.precio_unitario)); }
