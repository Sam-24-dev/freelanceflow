(function proposalModelFactory(globalScope) {
  ...
  function hasValidLegacyRawValues(proposal) { return isPlainObject(proposal) && CURRENCIES.includes(text(proposal.moneda).toUpperCase()) && validRawMoney(proposal.descuento) && validRawMoney(proposal.subtotal_general) && validRawMoney(proposal.total_propuesta) && Array.isArray(proposal.items) && proposal.items.every((item) => isPlainObject(item) && validRawMoney(item.cantidad) && validRawMoney(item.precio_unitario) && validRawMoney(item.subtotal_item)); }
  ...
  function readProposalStorage(storage) { try {
    ...
    if (Array.isArray(parsed)) {
      if (!parsed.every(hasValidLegacyRawValues)) return { ok: false, proposals: [] };
