"""Read-only active-workspace project directory with session-bound cursors."""
...
from projects.services import ProjectAccessDenied, get_projects_for_workspace
...
CURSOR_SESSION_KEY = "api.projects.cursors"
CURSOR_VERSION = "v1"
CURSOR_SIGNER = TimestampSigner(salt="api.projects.cursor.v1")
READ_FIELDS = (
...
class ProjectListView(JsonMethodView):
    ...
    def get(self, request):
        ...
        try:
            ...
            projects = get_projects_for_workspace(context)
        except (WorkspacePermissionDenied, ProjectAccessDenied):
        ...
        projects = projects.annotate(
            ...
            proposal_public_id=F("proposal__public_id"),
            proposal_title=F("proposal__title"),
        ).order_by("proposal__title", "pk")
        if cursor_value is not None:
            projects = projects.filter(
                Q(proposal__title__gt=cursor_title)
                | Q(proposal__title=cursor_title, pk__gt=cursor_pk)
            )
        rows = list(projects.values(*READ_FIELDS, "pk")[: CURSOR_PAGE_SIZE + 1])
