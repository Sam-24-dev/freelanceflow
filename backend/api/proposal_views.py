"""Read-only active-workspace proposal directory with session-bound cursors."""
...
from proposals.services import ProposalAccessDenied, get_proposals_for_workspace
...
CURSOR_SESSION_KEY = "api.proposals.cursors"
CURSOR_VERSION = "v1"
CURSOR_SIGNER = TimestampSigner(salt="api.proposals.cursor.v1")
READ_FIELDS = (
    "public_id", "client_public_id", "client_legal_name", "title",
    "issued_on", "valid_until", "status", "archived_at",
)
...
def _read_cursor(request, context, value):
    try:
        ...
        if (
            ...
            or not isinstance(stored.get("title"), str)
            or not isinstance(stored.get("pk"), int)
            or stored["pk"] <= 0
        ):
            raise ValueError
        return stored["title"], stored["pk"], None
...
class ProposalListView(JsonMethodView):
    ...
    def get(self, request):
        ...
        try:
            ...
            proposals = get_proposals_for_workspace(context)
        except (WorkspacePermissionDenied, ProposalAccessDenied):
        ...
        if cursor_value is not None:
            proposals = proposals.filter(Q(title__gt=cursor_title) | Q(title=cursor_title, pk__gt=cursor_pk))
        rows = list(proposals.values(*READ_FIELDS, "pk")[: CURSOR_PAGE_SIZE + 1])
